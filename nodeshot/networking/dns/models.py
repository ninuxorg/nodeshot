...
class LayerToDns(models.Model):
    """
    OneToOne tables for customize automatic DNS names
    """
    layer                = models.OneToOneField('layers.Layer', db_index=True, unique=True)
    value               = models.SlugField(_('value'), max_length=20)

class NodeToDns(models.Model):
    layer                = models.OneToOneField('nodes.Node', db_index=True, unique=True)
    value               = models.SlugField(_('value'), max_length=20)

class DeviceToDns(models.Model):
    layer                = models.OneToOneField(Device, db_index=True, unique=True)
    value               = models.SlugField(_('value'), max_length=20)

class InterfaceToDns(models.Model):
    layer                = models.OneToOneField(Interface, db_index=True)
    value               = models.SlugField(_('value'), max_length=20)
