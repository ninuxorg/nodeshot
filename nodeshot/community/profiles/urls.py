...
urlpatterns = patterns('nodeshot.community.profiles.views',
    ...
    url(r'^profiles/(?P<username>[-.\w]+)/nodes/$',
        'user_nodes',
        name='api_user_nodes'),
    url(r'^profiles/(?P<username>[-.\w]+)/social-links/$',
        'user_social_links_list',
        name='api_user_social_links_list'),
    url(r'^profiles/(?P<username>[-.\w]+)/social-links/(?P<pk>[0-9]+)/$',
        'user_social_links_detail',
        name='api_user_social_links_detail'),
...
if settings.NODESHOT['SETTINGS'].get('PROFILE_EMAIL_CONFIRMATION', True):
    urlpatterns += patterns('nodeshot.community.profiles.views',
        ...
        url(r'^account/email/(?P<pk>[0-9]+)/$',
            'account_email_detail',
            name='api_account_email_detail'),
        url(r'^account/email/(?P<pk>[0-9]+)/resend-confirmation/$',
            'account_email_resend_confirmation',
            name='api_account_email_resend_confirmation'),
    )
